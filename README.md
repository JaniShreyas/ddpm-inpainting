...
The framework, out of the box, supports MNIST digits and CIFAR10 as datasets (you can obviously easily implement a custom dataloader and add it to the data factory), Flow matching with Heun sampler, EDM, DDPM training algorithm with DDIM sampling, and backbones for UNet and JiT (from the Back to Basics paper: (https://arxiv.org/pdf/2511.13720). There will be more models and backbones (DiT backbone for example) and datasets (CelebA) added in the future.
